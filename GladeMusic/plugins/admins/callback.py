...
from GladeMusic import YouTube, app
from GladeMusic.core.call import Glade
from GladeMusic.misc import SUDOERS, db
from GladeMusic.utils.database import (is_active_chat,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Glade.pause_stream(chat_id)
        ...
        await Glade.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Glade.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Glade.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Glade.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Glade.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
